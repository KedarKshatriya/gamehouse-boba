...
document.querySelector('#retry').addEventListener('click', async () => {
  ...
  // await createNFT(counterDOM.innerHTML);
      ...
      if (chickenMaxX > carMinX && chickenMinX < carMaxX) {
        if (killCounter > 0) {
          ...
          if (counter == 0) {
            ...
            imgDat1 = imgData.replace(strMime, strDownloadMime);
            renderer.domElement.toBlob(function (blob) {
              currentScore = counterDOM.innerHTML;
              ...
              const options = {
                ...
                headers: {
                  // find this new
                  Authorization: 'a922fc09-c9de-46f1-9e75-6227e501d96c',
                },
              };
              fetch('https://api.nftport.xyz/v0/files', options)
                ...
                .then((responseJson) => {
                  if (responseJson.ipfs_url) {
                    const metadata = {
                      name: 'Crypto Chicken Run GAME',
                      description: `Scored ${currentScore}`,
                    ...
                    fetch('https://api.nft.storage/upload', options)
                      ...
                      .then((responseJson) => {
                        if (responseJson.ok) {
                          ...
                          fetch('/apiRoute/mintNft', options).then(
                            async (response) => {
                              let resObj = await response.json();
                              console.log(`Txhash link: https://blockexplorer.rinkeby.boba.network/tx/${resObj.hash}`)
                            }
                          ).catch(
                              (error) => {
                                console.log(error);
                  ...
                  // Handle the response
                  delScores(accountAddress[0]);
                  console.log(responseJson);
                });
            });
            // createNft(counterDOM.innerHTML, imgDat1);
            counter++;
          }
